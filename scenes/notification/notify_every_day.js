    ...
    return new Scene('notify_e_d',
        ...
        ctx=>{
            if (ctx.message.payload) {
                ...
                switch (payload) {
                    case 'Отключить':
                        const sql = `UPDATE ${process.env.DB_TABLE} SET notify_e_d = false WHERE vk_id = ${ctx.message.from_id}`;
        ...
        ctx=>{
            if (ctx.message.payload) {
                ...
                if (time_str_matched) {
                    const sql =
                        `UPDATE ${process.env.DB_TABLE} SET notify_time = '${time_str_matched[0]}', notify_groups = '{${ctx.session.marked_groups.join(',')}}', notify_e_d=true WHERE vk_id = ${ctx.message.from_id}`;
