...
async function task_mailing(bot, message, attachment) {
    ...
    for (let i = 1; i < TOTAL_CONV;i++){
        ...
        await bot.execute('messages.send', sending_params).catch(error => console.error(`task_mailing error: ${error}`));
    }
}
        ...
        send_image_changes()(data_changes, [], async (err, buffer)=>{
            if (err){
                ...
                await saveImageIntoVk(buffer,bot,async (photo_data)=>{
                    await task_mailing(bot, null, `photo${photo_data[0].owner_id}_${photo_data[0].id}`);
                });
            }
        });
    }
